"""AcknowledgementHandler — student's thanks page.

Detects pages whose title block contains 'ACKNOWLEDGEMENT'. Emits the title
as Heading 1, body as justified Normal paragraphs, and any 'Place: / Date: /
Reg No.' signature block at the bottom.
"""
...
_TITLE_VARIANTS = ("ACKNOWLEDGEMENT", "ACKNOWLEDGEMENTS")
_SIG_PREFIXES   = ("Place:", "Date:", "Reg No", "Reg. No", "EN No", "Enrollment")
...
def _looks_like_author_name(s: str) -> bool:
    """Last line of acknowledgement is typically the student's name."""
    words = s.split()
...
class AcknowledgementHandler(SectionHandler):
    """Pages whose first heading-sized block reads ACKNOWLEDGEMENT."""
    ...
    def applies_to(self, blocks, page_no, ctx) -> bool:
        if page_no >= ctx.front_matter_page_limit:
            return False
        # Scan first 5 blocks for the title — works for both Word PDFs
        # (heading-sized title) and Canva PDFs (uniform-font, title is
        # just an ALL-CAPS short line at the top).
    ...
    def render(self, doc: "Document", blocks, page_no, ctx: "Context") -> None:
        doc.add_page_break()
        title = doc.add_heading("ACKNOWLEDGEMENT", level=1)
