...
| Print server | `print_server.py` | 3005 | Staff auth (PBKDF2+salt PIN), `/print` `/quote` `/service-quote` `/new-service` `/finishing-send` `/finishing-receive` `/finishing-return` `/finishing-incoming` `/staff-login` `/staff-logout` `/printers`; drives SumatraPDF |
