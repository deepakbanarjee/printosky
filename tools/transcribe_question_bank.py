# -*- coding: utf-8 -*-
"""
Transcribe the second handwritten Malayalam PDF document (Question Bank, 69 pages) page-by-page.
Uses the visual reference page from the first document and a contextual glossary for maximum accuracy.
...
PDF_PATH = r"C:\Users\user\Downloads\PDF_compressed (1).pdf"
REF_PDF_PATH = r"C:\Users\user\Downloads\🌺ഹൈസ്കൂൾ മലയാളം പരീക്ഷാസഹായി By DM🌺.pdf.pdf"
OUT_PATH = r"C:\Users\user\Downloads\question_bank_transcript.txt"
TEMP_DIR = r"d:\PY\printosky\_tmp_pages_qb"

# Reference page index from the first document (Page 10 is index 9)
REF_PAGE_IDX = 9
...
def main():
    ...
    if not os.path.exists(REF_PDF_PATH):
        print(f"ERROR: Reference PDF file not found at: {REF_PDF_PATH}")
        sys.exit(1)

    os.makedirs(TEMP_DIR, exist_ok=True)

    print("Opening target PDF...")
    doc = fitz.open(PDF_PATH)
    total_pages = len(doc)
    print(f"Total pages in target PDF: {total_pages}")

    # Generate reference image from the reference PDF
    ref_img_path = os.path.join(TEMP_DIR, f"ref_page_{REF_PAGE_IDX}.png")
    if not os.path.exists(ref_img_path):
        print(f"Extracting reference page {REF_PAGE_IDX + 1} from reference PDF...")
